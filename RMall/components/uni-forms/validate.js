...
class RuleValidator {
    ...
    validateRule(key, value, data, allData) {
        var result = null
        ...
        for (var i = 0; i < rules.length; i++) {
            ...
            if (rule.validateFunction) {
                result = await
                this.validateFunction(rule, value, data, allData, vt)
                if (result !== null) {
                    break
                }
            }
    ...
    validateFunction(rule, value, data, allData, vt) {
        ...
        try {
            ...
            const res = await
            rule.validateFunction(rule, value, allData || data, (message) = > {
...
class SchemaValidator extends RuleValidator {
    ...
    validate(data, allData) {
        ...
        if (!result) {
            result = await
            this.invokeValidate(data, false, allData)
        }
    ...
    validateAll(data, allData) {
        ...
        if (!result) {
            result = await
            this.invokeValidate(data, true, allData)
        }
        return result
    }
    ...
    validateUpdate(data, allData) {
        ...
        if (!result) {
            result = await
            this.invokeValidateUpdate(data, false, allData)
        }
        return result.length ? result[0] : null
    }
    ...
    invokeValidate(data, all, allData) {
        ...
        for (let key in schema) {
            ...
            this.validateRule(value, data[key], data, allData)
    ...
    invokeValidateUpdate(data, all, allData) {
        ...
        for (let key in data) {
            let errorMessage = await
            this.validateRule(this._schema[key], data[key], data, allData)
            if (errorMessage != null) {
                result.push({
                    key,
                    errorMessage
